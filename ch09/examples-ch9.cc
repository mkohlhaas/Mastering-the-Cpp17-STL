...
namespace ex12
{
    ...
    test()
    {
        ...
        // ex12
        char buf[13];
        int  needed = snprintf(buf, sizeof buf, "There are %d piano tuners in %s", tuners, where);
        assert(needed == 37);
...
namespace ex14
{
    std::string better_format(const char *fmt, ...) __attribute((format(printf, 1, 2)));
    // ex14
    std::string
    better_format(const char *fmt, ...)
    {
        va_list ap;
        va_start(ap, fmt);
...
namespace ex19
{
    ...
    test()
    {
        try
        {
            std::cout << "There are " << computation_that_may_throw() << "piano tuners here.\n";
        }
        catch (...)
        {
...
namespace ex20
{
    ...
    // ex20
    void
    pad(std::ostream &os, size_t from, size_t to)
    {
        char ch = os.fill();
    ...
    auto &
    operator<<(std::ostream &os, WidthSetter w)
    {
        os.width(w.n);
        return os;
    }
    ...
    auto &
    operator<<(std::ostream &os, Manip f)
    {
        return f(os);
    }
    ...
    test()
    {
        ...
        std::cout << std::setw(8) << std::left << where << "." << std::setw(4) << std::right << std::hex << tuners
                  << "\n";
        // "Chicago .  e1"
        // dex22
...
namespace ex29
{
    ...
    OutputIt
    do_quote(InputIt begin, InputIt end, OutputIt dest)
    {
        *dest++ = '"';
        while (begin != end)
        {
    ...
    auto &
    operator<<(std::ostream &os, const quoted &q)
    {
        do_quote(q.m_view.begin(), q.m_view.end(), std::ostreambuf_iterator<char>(os));
        return os;
    }
    // dex30
    void
...
namespace ex33
{
    ...
    test()
    {
        ...
        *r.ptr = '\0';
        ...
        if (false)
        {
            ...
            // ex35
            std::string response = "Content-Length: " + std::to_string(body.size()) + "\r\n" + "\r\n" + body;
            // dex35
        ...
        r      = std::to_chars(buffer, std::end(buffer), floatvalue, std::chars_format::scientific, 6);
        r      = std::to_chars(buffer, std::end(buffer), floatvalue, std::chars_format::fixed, 6);
        r      = std::to_chars(buffer, std::end(buffer), floatvalue, std::chars_format::general, 6);
        *r.ptr = '\0';
        // available since C++17 in <charconv>
        // locale-independent by design
        // non-allocating
    ...
    // ex36
    char *
    write_string(char *p, char *end, const char *from)
    {
        while (p != end && *from != '\0')
        {
            *p++ = *from++;
    ...
    char *
    write_response_headers(char *p, char *end, std::string body)
    {
...
namespace ex39
{
    ...
    test()
    {
        ...
        r = std::from_chars(buffer, buffer + 2, floatvalue, std::chars_format::general);
        // sets r.ec != 0 on any error
        // available since C++17 in <charconv>
        // locale-independent by design
        // non-allocating
