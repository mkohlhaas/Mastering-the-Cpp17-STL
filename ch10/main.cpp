...
{
    void
    process_command(const std::string &)
    {
    }
    void
    test()
...
namespace ex8
{
    static int count = 0;
    void
    process_command(const char *, const char *)
    {
        ++count;
    }
    // ex8
    void
    parse(const char *p, const char *end)
    {
...
namespace ex16
{
    // ex16
    auto
    get_all_matches(const char *begin, const char *end, const std::regex &rx, bool be_correct)
    {
        auto flags = be_correct ? std::regex_constants::match_prev_avail : std::regex_constants::match_default;
...
namespace ex18
{
    // ex18
    auto
    get_all_matches(const char *begin, const char *end, const std::regex &rx)
    {
        std::vector<std::string> result;
...
namespace ex19
{
    // ex19
    auto
    get_tokens(const char *begin, const char *end, const std::regex &rx)
    {
        std::vector<std::string> result;
        using It = std::cregex_iterator;
...
namespace ex21
{
    ...
    test()
    {
        ...
        std::copy(TokIt(input.begin(), input.end(), rx, {1, 2}), TokIt(), std::back_inserter(v));
        assert(!v[0].matched);
        assert(v[1] == "abc");
        assert(v[2] == "123");
        assert(!v[3].matched);
...
namespace ex24
{
    ...
    template <class F>
    std::string
    regex_replace(std::string_view haystack, const std::regex &rx, const F &f)
    {
        std::string result;
    ...
    test()
    {
        ...
        auto t = regex_replace(s, std::regex("\\bstd::(\\w+)"), [](auto &&m) {
            ...
            std::transform(m[1].first, m[1].second, begin(result), ::toupper);
            return result;
        });
        assert(t == "SORT(BEGIN(v), END(v))");
    }
