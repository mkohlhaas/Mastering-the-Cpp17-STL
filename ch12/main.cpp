...
namespace std
{
    ...
    from_chars_result
    from_chars(const char *, const char *, int &, int = 10)
    {
        return {};
    }
    ...
    namespace std
    {
        ...
        class error_category
        {
          public:
            ...
            bool
            operator==(const std::error_category &rhs) const
            {
                return this == &rhs;
            }
        };
...
namespace ex13
{
    ...
    namespace FinickyFS
    {
        ...
        struct ErrorCategory : std::error_category
        {
            ...
            std::string
            message(int err) const override
            {
                switch (err)
                {
                case 0:
        ...
        std::error_code
        make_error_code(Error err) noexcept
        {
    ...
    test()
    {
        ...
        std::error_code ec = make_error_code(FinickyFS::Error::forbidden_word);
...
namespace FinickyFS
{
    ...
    struct ConditionCategory : std::error_category
    {
        ...
        std::string
        message(int cond) const override
        {
            switch (cond)
            {
            case 0:
        ...
        equivalent(const std::error_code &ec, int cond) const noexcept override
        {
            ...
            case 0:
                return !ec;
            case 1:
                return is_malformed_name(ec);
            }
            throw Unreachable();
        }
        static ConditionCategory &
        instance()
        {
    ...
    std::error_condition
    make_error_condition(Condition cond) noexcept
    {
...
{ // must be same as above
    ...
    namespace FinickyFS
    {
        // ex18
        bool
        is_malformed_name(std::error_code ec)
        {
            return (ec == FinickyFS::Error::forbidden_character || ec == FinickyFS::Error::forbidden_word ||
...
namespace ex16
{
    ...
    {
        // ...
        template <class E, class = enable_if_t<is_error_code_enum_v<E>>>
...
#define file_size file_size2
    ...
    // dex21
    uintmax_t
    file_size(const fs::path &, std::error_code &) noexcept
    {
        return -1;
    }
#undef file_size
...
{
    template <>
    struct is_error_code_enum<::ex22::detail::Error> : true_type
    {
    };
} // namespace std
...
#define file_size file_size2
    ...
    uintmax_t
    file_size(const fs::path &p, std::error_code &ec) noexcept
    {
        uintmax_t size = -1;
        try
        {
...
#undef file_size
    ...
    std::error_code
    current_exception_to_error_code()
    {
        try
        {
            throw;
...
namespace ex22::detail
{
    ...
    std::string
    ErrorCategory::message(int err) const
    {
        switch (err)
        {
        case 0:
...
namespace ex24
{
    ...
    {
      public:
        using value_type  = std::conditional_t<IsWindows, wchar_t, char>;
...
namespace ex26
{
    ...
    // ex27
    static fs::path
    operator+(fs::path a, const fs::path &b)
    {
        a += b;
        return a;
    }
...
#define X(x) auto x = path.x();
        X(root_path)
        X(root_name) X(root_directory) X(relative_path) X(parent_path) X(filename) X(stem) X(extension) X(is_absolute)
            X(is_relative)
#undef X
            // ex26
...
namespace ex28
{
    ...
    test()
    {
        ...
        assert((v == std::vector<fs::path>{"/", "foo", "bar", "baz.txt"}));
        // dex28
    }
} // namespace ex28
...
namespace ex29
{
    ...
    test()
    {
        ...
        std::cout << entry.path() << " has size " << entry.file_size() << "\n";
        // dex29
    }
} // namespace ex29
#endif // USE_BOOST
...
namespace ex30
{
    ...
    test()
    {
        ...
        {
            std::cout << entry.path().string() << ": " << file_size(entry.path()) << " bytes\n";
        }
    }
#else
    void
...
namespace ex31
{
    ...
    template <class F>
    void
    walk_down(const fs::path &p, const F &callback)
    {
